...
#========= Load data =========#
gene_statuses <- read.table(paste0(base_dir,'/HMF_update/analysis/hr_gene_def/hmf_gene_statuses.txt.gz'),sep='\t',header=T)
...
gene_statuses_brca <- subset(gene_statuses, hgnc_symbol %in% c("BRCA1","BRCA2")) %>% 
   .[order(.$is_def, decreasing=T),]

gene_statuses_brca <- cbind(
   hrd = chord_pred[match(gene_statuses_brca$sample,rownames(chord_pred)),'hrd'],
   gene_statuses_brca
)
...
def_samples  <- list(
   luan = gene_statuses_brca %>% .[.$is_def == T,'sample_name'] %>% unique(),
